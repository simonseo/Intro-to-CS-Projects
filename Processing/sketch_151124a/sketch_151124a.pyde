...
class Game:
    def __init__ (self, w,h,g):
        ...
        self.enemies = []
        self.enemies.append(Spongebob(800, 200, self.g, 30))
        self.platforms=[]
...
class Mario(Creature):
    ...
    def update(self):
        ...
        for p in game.platforms:
            ...
            if  p.x <= self.x-game.x <=p.x+p.w and self.y+self.r <= p.y+p.h and self.vy > 0:
                self.g = p.y
                onPlatform = True
                break
        ...
        if self.x >= game.w/2 and self.vx >0:
            game.x += self.vx
        if (self.x < game.w/2 or self.vx < 0) and game.x > 0 and self.vx < 0:
            game.x += self.vx
    ...
    def display(self):
        ...
        ellipse (self.x-game.x, self.y, 2*self.r, 2*self.r)
        stroke(255,0,0)
        line(self.x-game.x-self.r, self.g,self.x-game.x+self.r, self.g)
        if self.vx != 0:
            self.f = self.f+0.1
        else:
            self.f=0
        ...
        if self.lastV > 0:
            image(self.img,self.x-game.x-self.r,self.y-self.r, self.r*2,self.r*2,int(self.f)%self.F*153,0,int(self.f)%self.F*153+153,216)
        else:
            image(self.img,self.x-game.x-self.r,self.y-self.r, self.r*2,self.r*2,int(self.f)%self.F*153+153,0,int(self.f)%self.F*153,216)
...
class Spongebob(Creature):
    ...
    def display(self):
        ...
        if self.lastV > 0:
            image(self.img,self.x-game.x-self.r,self.y-self.r, self.r*2,self.r*2,int(self.f)%self.F*60,0,int(self.f)%self.F*60+60,59)
        else:
            image(self.img,self.x-game.x-self.r,self.y-self.r, self.r*2,self.r*2,int(self.f)%self.F*60+60,0,int(self.f)%self.F*60,59)


class Jellyfish(Creature):
    def __init__ (self, x, y, g, r, x1, x2):
        super().__init__(self, x, y, g, r)
        self.img = [loadImage(path+'/Jellyfish[0].png'),loadImage(path+'/Jellyfish[1].png'),loadImage(path+'/Jellyfish[2].png'),loadImage(path+'/Jellyfish[3].png')]
        self.F = len(self.img)
        self.f = 0
        self.x1 = x1
        self.x2 = x2
