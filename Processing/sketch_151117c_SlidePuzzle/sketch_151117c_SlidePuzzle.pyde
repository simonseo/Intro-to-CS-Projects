...
class Board:
    ...
    def display(self):
        for tile in self.board:
            image(self.img, tile.col * 100, tile.row * 100, 99, 99, tile.col * 100, tile.row * 100, tile.col * 100 + 99, tile.row * 100 + 99)
             #image(img, posx, posy, scalex, scaley, cropx1, cropy1, cropx2, cropy2)
        ...
        for row in range(self.numRow):
            for col in range(self.numCol):
                if row == self.numRow - 1 and col == self.numCol - 1:
                    ...
                    self.board.append(
                        Tile(row, col, 1 + row * self.numCol + col))
